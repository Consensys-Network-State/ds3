// Source: https://github.com/radix-ui/themes/blob/main/packages/colors/src/generate-radix-colors.ts
...
const lightColorsObject = getColorScaleObject({
  isDarkMode: false,
  name: 'blue',
...
interface GetColorScaleObjectParams {
...
export const getColorScaleObject = ({
	scale,
	scaleWideGamut,
	scaleAlpha,
	scaleAlphaWideGamut,
	...
	surface,
	surfaceWideGamut,
}: GetColorScaleObjectParams) => {
	const baseColors = Object.fromEntries(
		scale.map((value, index) => [`${index + 1}`, value])
	);

	const alphaColors = Object.fromEntries(
		scaleAlpha.map((value, index) => [`a${index + 1}`, value])
	);

	const wideGamutColors = Object.fromEntries(
		scaleWideGamut.map((value, index) => [`${index + 1}`, value])
	);

	const wideGamutAlphaColors = Object.fromEntries(
		scaleAlphaWideGamut.map((value, index) => [`a${index + 1}`, value])
	);
	...
	return {
		...
		wideGamut: {
			...wideGamutColors,
			contrast,
			surface: surfaceWideGamut,
			indicator: scaleWideGamut[8],
			track: scaleWideGamut[8],
		},
		alpha: alphaColors,
		wideGamutAlpha: wideGamutAlphaColors,
	};
};